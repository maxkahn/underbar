(function() {
  ...
  _.last = function(array, n) {
    //Assuming n is an integer. Multiple ways we could extend to negatives.
      if (n === undefined) {
        ...
        var shortarr = array.slice();
        shortarr.shift();
        return _.last(shortarr, n);
      }
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
      if (arguments[2] == undefined) {
        //this case doesn't work for collection an object… but there is no first element, then.
        var acc = collection[0];
        collection.shift();
        return _.reduce(collection, iterator, acc);
      }
      else {        
        _.each(collection, function (item) {
  ...
  _.every = function(collection, iterator) {
    ...
    //If no callback provided, iterator is taken to be _.identity.
  ...
  _.sortBy = function(collection, iterator) {
      ...
      collection = _.reject(collection, function(a) {
          return a == undefined;
          });
      if (collection.length == 0)
        return [];
      else if (collection[0][iterator] == undefined)
        range = _.map(collection, iterator);
      else range = _.map(collection, function(obj) {
        return obj[iterator];
      });
      var pairs = [];
      for (var i = 0; i < collection.length; i++) {
